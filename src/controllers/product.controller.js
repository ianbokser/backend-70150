...
export const getProductById = async (req, res, next) => {
  try {
    ...
    if (!response) res.status(404).json({ msg: `No se puede encontrar el producto ${pid}` });
...
export const deleteProduct = async (req, res, next) => {
  try {
    ...
    if (!productToDelete) res.status(404).json({ msg: "No se puede eliminar producto" });
    else res.status(200).json({ msg: `El producto id: ${pid} fue eliminado` });
  } catch (error) {
    next(error.message);
  }
};